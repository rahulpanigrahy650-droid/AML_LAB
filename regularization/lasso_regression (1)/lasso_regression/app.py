...
def lasso_gradient_descent(x, y, w, b, learning_rate, iterations, alpha):
    """
    Lasso (L1) Regression using Gradient Descent.
    Cost = MSE + alpha * |w|
    Uses a subgradient (sign of w) since |w| is not differentiable at 0.
    """
    ...
    for i in range(1, iterations + 1):
        ...
        dw = (-2 / n) * np.sum(x * (y - y_pred)) + alpha * np.sign(w)
        ...
        cost = mse + alpha * abs(w)  # MSE + L1 penalty
...
def index():
    ...
    if request.method == "POST":
        ...
        final_w, final_b, history = lasso_gradient_descent(
        ...
        if show_graphs:
            ...
            plt.plot(x[sort_index], y_pred[sort_index], color="green", label="Lasso Regression Line")
