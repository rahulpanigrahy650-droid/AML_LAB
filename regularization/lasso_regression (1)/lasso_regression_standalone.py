...
class LassoRegression:
    def __init__(self, learning_rate=0.01, n_iter=1000, alpha=0.1):
        ...
        self.cost_history = []   # stores cost (MSE + L1 penalty) at every iteration
    ...
    def fit(self, x, y):  # x_train, y_train
        ...
        for i in range(self.n_iter):
            ...
            # step 3 - calc gradient (MSE part + L1 subgradient on weights)
            db = (1 / m) * np.sum(y_pred - y)
            dw = (1 / m) * np.dot(x.T, (y_pred - y)) + self.alpha * np.sign(self.weights)

            # step 4 - convergence theorem - params update
            self.bias -= self.lr * db
            self.weights -= self.lr * dw

            # step 5 - track cost for this iteration (MSE + L1 penalty)
            mse = (1 / m) * np.sum((y_pred - y) ** 2)
            cost = mse + self.alpha * np.sum(np.abs(self.weights))
...
if __name__ == "__main__":
    ...
    # -------- Train model --------
    model = LassoRegression(learning_rate=0.01, n_iter=1000, alpha=0.1)
    model.fit(x, y)
    ...
    plt.plot(x[sort_index, 0], y_pred[sort_index], color="green", label="Lasso Regression Line")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("lasso_regression_fit.png")
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("lasso_regression_cost.png")
    plt.show()
